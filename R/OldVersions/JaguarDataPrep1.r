...
X117=subset(jaguar_df,id=='117')
 ...
 #####################################################################################################################
 # Other tests (whithout adehabitat and move)  but issue with the Event_ID
...
#'  
#' setwd("https://github.com/piLaboratory/jaguar-codes/tree/master/R")  # <------------------- ???
...
#' 
#' mdo <- getURL("https://github.com/piLaboratory/jaguar-codes/tree/master/data/mov.data.org.txt") # <---- ???
#' mov.data.org <- read.delim(text = mdo)                                                          # <---- ???  
mov.data.org <- read.delim(file="D:/Documents/GitHub/jaguar-codes/data/mov.data.org.txt") # <---------------------------
mov.data.org <- dplyr::select(mov.data.org, -(individual.taxon.canonical.name:tag.local.identifier))
#'
#' Add Individual info  (see older development files for details)
#'
#' info <- read.delim(file="data/info.txt") # <----------------------------------------------------------- ???
info <- read.delim(file="D:/Documents/GitHub/jaguar-codes/data/info.txt")           # <---------------------------
#'
#' Merge movement with individual info/parameters
merged<- merge(mov.data.org,info)
mov.data.org <- merged 
...
jaguar_df <-  mov.data.org

colnames(jaguar_df)[colnames(jaguar_df)=="individual.local.identifier..ID."] <- "id"

colnames(jaguar_df)[colnames(jaguar_df)=="local_time"] <- "date"
colnames(jaguar_df)[colnames(jaguar_df)=="location.lat"] <- "y"
colnames(jaguar_df)[colnames(jaguar_df)=="location.long"] <- "x"

age <- as.numeric(levels(jaguar_df$age))[jaguar_df$age]
weight <- as.numeric(levels(jaguar_df$weight))[jaguar_df$weight]
jaguar_df$id <- as.factor(jaguar_df$id)  
jaguar_df$age <- age
jaguar_df$weight <- weight

jaguar_df$timestamp  <- NULL
jaguar_df$timestamp.posix.GMT  <- NULL
jaguar_df$collar_type <- NULL  
jaguar_df$brand <- NULL
jaguar_df$study.name <- NULL
jaguar_df$timezone  <- NULL
#' jaguar_df$timestamp.posix <- NULL
jaguar_df$week <- as.numeric(strftime(as.POSIXlt(jaguar_df$timestamp.posix),format="%W"))
jaguar_df$day <- as.numeric(strftime(as.POSIXlt(jaguar_df$timestamp.posix),format="%j"))
jaguar_df$year <- as.numeric(strftime(as.POSIXlt(jaguar_df$timestamp.posix),format="%Y"))
jaguar_df$hour <- as.numeric(strftime(as.POSIXlt(jaguar_df$timestamp.posix),format="%H"))
...
# day  7 to 16  and  night 19 to 4 and  sun riseset 5,6,17,18
jaguar_df$month=as.numeric(substr(jaguar_df$date,6,7))
jaguar_df$period=ifelse(jaguar_df$hour==7,"day",ifelse(jaguar_df$hour==8,"day",ifelse(jaguar_df$hour==9,"day",ifelse(jaguar_df$hour==10,"day", ifelse(jaguar_df$hour==11,"day", ifelse(jaguar_df$hour==12,"day", ifelse(jaguar_df$hour==13,"day",
ifelse(jaguar_df$hour==14,"day",ifelse(jaguar_df$hour==15,"day", ifelse(jaguar_df$hour==16,"day",ifelse(jaguar_df$hour==19,"night",ifelse(jaguar_df$hour==20,"night",ifelse(jaguar_df$hour==21,"night",ifelse(jaguar_df$hour==22,"night",
ifelse(jaguar_df$hour==23,"night",ifelse(jaguar_df$hour==0,"night",ifelse(jaguar_df$hour==1,"night",ifelse(jaguar_df$hour==2,"night",ifelse(jaguar_df$hour==3,"night",ifelse(jaguar_df$hour==4,"night","riseset"))))))))))))))))))))
